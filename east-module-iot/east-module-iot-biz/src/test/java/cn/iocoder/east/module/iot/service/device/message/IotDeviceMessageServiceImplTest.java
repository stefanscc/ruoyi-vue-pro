...
import cn.iocoder.east.framework.common.exception.ServiceException;
import cn.iocoder.east.framework.common.pojo.PageResult;
import cn.iocoder.east.framework.test.core.ut.BaseMockitoUnitTest;
import cn.iocoder.east.module.iot.controller.admin.device.vo.message.IotDeviceMessagePageReqVO;
...
import cn.iocoder.east.module.iot.core.topic.event.IotDeviceEventPostReqDTO;
import cn.iocoder.east.module.iot.core.topic.property.IotDevicePropertyPackPostReqDTO;
import cn.iocoder.east.module.iot.core.topic.property.IotDevicePropertyPostReqDTO;
...
import cn.iocoder.east.module.iot.dal.dataobject.device.IotDeviceMessageDO;
import cn.iocoder.east.module.iot.dal.tdengine.IotDeviceMessageMapper;
...
import cn.iocoder.east.module.iot.service.ota.IotOtaTaskRecordService;
...
public class IotDeviceMessageServiceImplTest extends BaseMockitoUnitTest {
    ...
    @Test
    public void testHandleUpstreamDeviceMessage_stateUpdatePassesReportTime() {
        LocalDateTime reportTime = LocalDateTime.of(2026, 6, 13, 11, 12, 13);
        IotDeviceDO device = buildDevice();
        IotDeviceMessage message = buildStateUpdateMessage(1, reportTime);

        try (MockedStatic<SpringUtil> springUtilMock = mockStatic(SpringUtil.class)) {
            springUtilMock.when(() -> SpringUtil.getBean(IotDeviceMessageServiceImpl.class)).thenReturn(service);
            service.handleUpstreamDeviceMessage(message, device);
        }

        verify(deviceService).updateDeviceState(device, 1, reportTime);
    }

    @Test
    public void testHandleUpstreamDeviceMessage_propertyPackPreservesReportTime() {
        LocalDateTime packReportTime = LocalDateTime.of(2026, 6, 13, 14, 15, 16);
        LocalDateTime explicitEventReportTime = LocalDateTime.of(2026, 6, 13, 16, 17, 18);
        IotDeviceDO gatewayDevice = buildDevice();
        IotDeviceDO subDevice = IotDeviceDO.builder().id(3L).build();
        IotDeviceMessage packMessage = buildPropertyPackMessage(packReportTime, explicitEventReportTime);
        when(deviceService.getDeviceFromCache("pk-sub", "dn-sub")).thenReturn(subDevice);

        try (MockedStatic<SpringUtil> springUtilMock = mockStatic(SpringUtil.class)) {
            springUtilMock.when(() -> SpringUtil.getBean(IotDeviceMessageServiceImpl.class)).thenReturn(service);
            service.handleUpstreamDeviceMessage(packMessage, gatewayDevice);
        }

        ArgumentCaptor<IotDeviceMessage> captor = ArgumentCaptor.forClass(IotDeviceMessage.class);
        verify(deviceMessageProducer, times(5)).sendDeviceMessage(captor.capture());
        List<IotDeviceMessage> messages = captor.getAllValues();

        IotDeviceMessage gatewayPropertyMessage = findPropertyMessage(messages, gatewayDevice.getId(), "temperature");
        assertEquals(packReportTime, gatewayPropertyMessage.getReportTime());

        IotDeviceMessage explicitEventMessage = findEventMessage(messages, gatewayDevice.getId(), "alert");
        assertEquals(explicitEventReportTime, explicitEventMessage.getReportTime());

        IotDeviceMessage fallbackEventMessage = findEventMessage(messages, gatewayDevice.getId(), "heartbeat");
        assertEquals(packReportTime, fallbackEventMessage.getReportTime());

        IotDeviceMessage subDevicePropertyMessage = findPropertyMessage(messages, subDevice.getId(), "humidity");
        assertEquals(packReportTime, subDevicePropertyMessage.getReportTime());

        IotDeviceMessage subDeviceEventMessage = findEventMessage(messages, subDevice.getId(), "subHeartbeat");
        assertEquals(packReportTime, subDeviceEventMessage.getReportTime());
    }

    @Test
    public void testHandleUpstreamDeviceMessage_subDeviceStatePackDelegatesToExpander() {
        IotDeviceDO gatewayDevice = buildDevice();
        ...
        subDeviceState.setIdentity(new IotDeviceIdentity("pk-sub", "dn-sub"));
        subDeviceState.setState(1);
        params.setSubDevices(List.of(subDeviceState));
        IotDeviceMessage message = buildMessage(IotDeviceMessageMethodEnum.SUB_DEVICE_STATE_PACK_POST.getMethod());
        message.setParams(params);

        try (MockedStatic<SpringUtil> springUtilMock = mockStatic(SpringUtil.class)) {
            springUtilMock.when(() -> SpringUtil.getBean(IotDeviceMessageServiceImpl.class)).thenReturn(service);
            service.handleUpstreamDeviceMessage(message, gatewayDevice);
        }

        verify(subDeviceStatePackExpander).expand(message, gatewayDevice);
        verifyNoInteractions(deviceMessageProducer);
    }
    ...
    private IotDeviceMessage buildStateUpdateMessage(Integer state, LocalDateTime reportTime) {
        IotDeviceMessage message = buildMessage(IotDeviceMessageMethodEnum.STATE_UPDATE.getMethod());
    ...
    private IotDeviceMessage buildPropertyPackMessage(LocalDateTime packReportTime, LocalDateTime explicitEventReportTime) {
        IotDevicePropertyPackPostReqDTO params = new IotDevicePropertyPackPostReqDTO();
        params.setProperties(Map.of("temperature", 26));

        IotDevicePropertyPackPostReqDTO.EventValue alertEvent = new IotDevicePropertyPackPostReqDTO.EventValue();
        alertEvent.setValue(Map.of("level", "HIGH"));
        alertEvent.setTime(LocalDateTimeUtil.toEpochMilli(explicitEventReportTime));
        IotDevicePropertyPackPostReqDTO.EventValue heartbeatEvent = new IotDevicePropertyPackPostReqDTO.EventValue();
        heartbeatEvent.setValue("ok");
        params.setEvents(Map.of("alert", alertEvent, "heartbeat", heartbeatEvent));

        IotDevicePropertyPackPostReqDTO.SubDeviceData subDeviceData = new IotDevicePropertyPackPostReqDTO.SubDeviceData();
        subDeviceData.setIdentity(new IotDeviceIdentity("pk-sub", "dn-sub"));
        subDeviceData.setProperties(Map.of("humidity", 60));
        IotDevicePropertyPackPostReqDTO.EventValue subHeartbeatEvent = new IotDevicePropertyPackPostReqDTO.EventValue();
        subHeartbeatEvent.setValue("sub-ok");
        subDeviceData.setEvents(Map.of("subHeartbeat", subHeartbeatEvent));
        params.setSubDevices(List.of(subDeviceData));

        IotDeviceMessage message = buildMessage(IotDeviceMessageMethodEnum.PROPERTY_PACK_POST.getMethod());
        message.setServerId("gateway-server");
        message.setParams(params);
        message.setReportTime(packReportTime);
        return message;
    }

    private IotDeviceMessage findPropertyMessage(List<IotDeviceMessage> messages, Long deviceId, String propertyName) {
        return messages.stream()
                .filter(message -> IotDeviceMessageMethodEnum.PROPERTY_POST.getMethod().equals(message.getMethod()))
                .filter(message -> deviceId.equals(message.getDeviceId()))
                .filter(message -> {
                    IotDevicePropertyPostReqDTO params = (IotDevicePropertyPostReqDTO) message.getParams();
                    return params.containsKey(propertyName);
                })
                .findFirst()
                .orElseThrow();
    }

    private IotDeviceMessage findEventMessage(List<IotDeviceMessage> messages, Long deviceId, String eventIdentifier) {
        Optional<IotDeviceMessage> message = messages.stream()
                .filter(item -> IotDeviceMessageMethodEnum.EVENT_POST.getMethod().equals(item.getMethod()))
                .filter(item -> deviceId.equals(item.getDeviceId()))
                .filter(item -> {
                    IotDeviceEventPostReqDTO params = (IotDeviceEventPostReqDTO) item.getParams();
                    return eventIdentifier.equals(params.getIdentifier());
                })
                .findFirst();
