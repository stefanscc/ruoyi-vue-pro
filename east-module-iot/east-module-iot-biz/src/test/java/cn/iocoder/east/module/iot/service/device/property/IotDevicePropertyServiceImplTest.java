...
import cn.iocoder.east.module.iot.dal.dataobject.thingmodel.model.ThingModelProperty;
...
public class IotDevicePropertyServiceImplTest extends BaseMockitoUnitTest {
    ...
    @Test
    public void testUpdateDeviceReportTimeAsync_usesUpdateIfNewer() {
        LocalDateTime reportTime = LocalDateTime.of(2026, 6, 13, 14, 15, 16);

        service.updateDeviceReportTimeAsync(1L, reportTime);

        verify(deviceReportTimeRedisDAO).updateIfNewer(1L, reportTime);
    }

    @Test
    public void testUpdateDeviceReportTimeAsync_olderReportTimeDoesNotOverwrite() {
        LocalDateTime newerReportTime = LocalDateTime.of(2026, 6, 13, 14, 15, 16);
        LocalDateTime olderReportTime = newerReportTime.minusMinutes(5);
        when(deviceReportTimeRedisDAO.updateIfNewer(1L, newerReportTime)).thenReturn(true);
        when(deviceReportTimeRedisDAO.updateIfNewer(1L, olderReportTime)).thenReturn(false);

        service.updateDeviceReportTimeAsync(1L, newerReportTime);
        service.updateDeviceReportTimeAsync(1L, olderReportTime);

        verify(deviceReportTimeRedisDAO).updateIfNewer(1L, newerReportTime);
        verify(deviceReportTimeRedisDAO).updateIfNewer(1L, olderReportTime);
    }
