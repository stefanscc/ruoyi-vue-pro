...
import cn.iocoder.east.framework.common.util.date.LocalDateTimeUtils;
...
import cn.iocoder.east.module.iot.controller.admin.statistics.vo.IotStatisticsDeviceMessageReqVO;
import cn.iocoder.east.module.iot.controller.admin.statistics.vo.IotStatisticsDeviceMessageSummaryByDateRespVO;
...
import cn.iocoder.east.module.iot.core.util.IotDeviceMessageUtils;
...
import static cn.iocoder.east.module.iot.enums.ErrorCodeConstants.DEVICE_DOWNSTREAM_FAILED_SERVER_ID_NULL;
...
public class IotDeviceMessageServiceImpl implements IotDeviceMessageService {
    ...
    private Object handleUpstreamDeviceMessage0(IotDeviceMessage message, IotDeviceDO device) {
        ...
        if (Objects.equal(message.getMethod(), IotDeviceMessageMethodEnum.STATE_UPDATE.getMethod())) {
            ...
            deviceService.updateDeviceState(device, state, message.getReportTime());
            return null;
        }
        // 网关批量上报子设备状态
    ...
    private void handlePackMessage(IotDeviceMessage packMessage, IotDeviceDO gatewayDevice) {
        LocalDateTime packReportTime = packMessage.getReportTime();
        // 1. 解析参数
        ...
        // 2. 处理网关设备（自身）的数据
        sendDevicePackData(gatewayDevice, packMessage.getServerId(), packReportTime, params.getProperties(), params.getEvents());
        ...
        for (IotDevicePropertyPackPostReqDTO.SubDeviceData subDeviceData : params.getSubDevices()) {
            try {
                ...
                sendDevicePackData(subDevice, null, packReportTime, subDeviceData.getProperties(), subDeviceData.getEvents());
            } catch (Exception ex) {
        ...
        if (MapUtil.isNotEmpty(events)) {
            for (Map.Entry<String, IotDevicePropertyPackPostReqDTO.EventValue> eventEntry : events.entrySet()) {
                ...
                IotDeviceMessage eventMsg = IotDeviceMessage.requestOf(
                        ...
                        .setReportTime(resolvePackEventReportTime(eventValue, packReportTime));
                deviceMessageProducer.sendDeviceMessage(eventMsg);
            }
        }
    }

    private LocalDateTime resolvePackEventReportTime(IotDevicePropertyPackPostReqDTO.EventValue eventValue,
                                                     LocalDateTime packReportTime) {
        return eventValue.getTime() != null ? LocalDateTimeUtil.of(eventValue.getTime()) : packReportTime;
    }
