...
import cn.iocoder.east.module.iot.core.topic.topo.IotDeviceTopoGetReqDTO;
...
import cn.iocoder.east.module.iot.gateway.serialize.json.IotJsonSerializer;
 ...
 * <p>使用步骤：
 * <ol>
 *     <li>启动 east-module-iot-gateway 服务（MQTT 端口 1883）</li>
 *     <li>运行以下测试方法：
 *         <ul>
...
public class IotGatewayDeviceMqttProtocolIntegrationTest {
    ...
    public void testStatePackPost() throws Exception {
        // 1. 连接并认证
        MqttClient client = connectAndAuth();
        log.info("[testStatePackPost][连接认证成功]");

        try {
            // 2.1 构建请求消息
            IotSubDeviceStatePackPostReqDTO.SubDeviceState subDeviceState = new IotSubDeviceStatePackPostReqDTO.SubDeviceState();
            subDeviceState.setIdentity(new IotDeviceIdentity(SUB_DEVICE_PRODUCT_KEY, SUB_DEVICE_NAME));
            subDeviceState.setState(1);
            subDeviceState.setTime(System.currentTimeMillis());
            IotSubDeviceStatePackPostReqDTO params = new IotSubDeviceStatePackPostReqDTO();
            params.setSubDevices(ListUtil.of(subDeviceState));
            IotDeviceMessage request = IotDeviceMessage.requestOf(
                    IotDeviceMessageMethodEnum.SUB_DEVICE_STATE_PACK_POST.getMethod(),
                    params);
